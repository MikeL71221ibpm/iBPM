...
// Performance middleware - compress responses
app.use((req, res, next) => {
  res.setHeader('Cache-Control', 'public, max-age=300'); // 5 min cache for static content
  next();
});
...
(async () => {
  ...
  httpServer.listen(port, "0.0.0.0", () => {
    ...
    // NOTE: Automatic extraction is triggered 2 seconds after CSV upload completes
    // See line 5182 in routes.ts - no manual intervention required
    ...
    try {
      pdfCleanupService.initialize();
      console.log('🧹 PDF cleanup service initialized - will clean PDFs older than 24 hours');
    } catch (error) {
      console.error('❌ Failed to initialize PDF cleanup service:', error);
    }
  });
})();
