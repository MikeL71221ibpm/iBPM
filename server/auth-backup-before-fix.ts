...
export function setupAuth(app: Express) {
  ...
  const isSecureEnvironment = isProduction || process.env.REPL_SLUG; // Replit deployments use HTTPS
  ...
  const sessionSettings: session.SessionOptions = {
    ...
    cookie: {
      ...
      httpOnly: true,
      sameSite: 'lax',
      secure: isSecureEnvironment, // Dynamic based on environment
  ...
  passport.use(
    new LocalStrategy({ usernameField: 'username' }, async (usernameOrEmail, password, done) => {
      ...
      try {
        ...
        // Try to find user by email first (case-insensitive using raw SQL)
        if (usernameOrEmail.includes('@')) {
          console.log(`Searching for user by email: ${usernameOrEmail}`);
        ...
        // If not found by email, try username (case-insensitive using raw SQL)
        if (!user) {
          ...
          if (usernameResults.rows.length > 0) {
            ...
            console.log(`User found by username: ${user.id}`);
          }
        }
