 ...
 * 
 * Usage: node server/bulk-symptom-comparison.mjs [path_to_symptom_file] [path_to_notes_file]
 * Example: node server/bulk-symptom-comparison.mjs ./attached_assets/Symptom_Segments_asof_4_30_25_MASTER.xlsx
 */
...
// Import the original symptom extractor (v3.0) dynamically
const originalExtractorPath = path.join(__dirname, 'utils', 'symptomExtractor.js');
const originalExtractor = await import(originalExtractorPath);
...
async function processNotesWithBothAlgorithms(notes, symptoms) {
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      ...
      // Import the V3.2 symptom matcher
      const v32MatcherPath = path.join(__dirname, 'utils', 'symptomMatcherV3_2.mjs');
      const v32Module = await import(v32MatcherPath);
      const v32Matcher = v32Module.refinedSymptomMatcher;
      
      for (const note of batch) {
        const v32Result = v32Matcher(note.note_text, symptoms, {
...
|--------|----------------|----------------|------------|----------|
| Total Symptoms Found | ${metrics.totalV30Matches} | ${metrics.totalV32Matches} | ${metrics.totalV32Matches - metrics.totalV30Matches > 0 ? '+' : ''}${metrics.totalV32Matches - metrics.totalV30Matches} | ${metrics.totalV30Matches ? ((metrics.totalV32Matches - metrics.totalV30Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}% |
| Notes With Symptoms | ${metrics.notesWithV30Matches} | ${metrics.notesWithV32Matches} | ${metrics.notesWithV32Matches - metrics.notesWithV30Matches > 0 ? '+' : ''}${metrics.notesWithV32Matches - metrics.notesWithV30Matches} | ${metrics.notesWithV30Matches ? ((metrics.notesWithV32Matches - metrics.notesWithV30Matches) / metrics.notesWithV30Matches * 100).toFixed(1) : 'N/A'}% |
...
${metrics.totalV32Matches > metrics.totalV30Matches
  ? `1. **Enhanced Detection**: V3.2 identified ${metrics.totalV32Matches - metrics.totalV30Matches} more symptoms (${metrics.totalV30Matches ? ((metrics.totalV32Matches - metrics.totalV30Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}% increase).`
  : `1. **Detection Changes**: V3.2 identified ${metrics.totalV30Matches - metrics.totalV32Matches} fewer symptoms (${metrics.totalV30Matches ? ((metrics.totalV30Matches - metrics.totalV32Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}% decrease).`}
...
${metrics.totalV32Matches > metrics.totalV30Matches
  ? `The V3.2 symptom matcher demonstrates significant improvements over V3.0, detecting ${metrics.totalV30Matches ? ((metrics.totalV32Matches - metrics.totalV30Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}% more symptoms across the clinical notes analyzed. The intentional preservation of duplicate symptoms in V3.2 helps represent symptom intensity, which was not captured in V3.0.`
  : `The V3.2 symptom matcher demonstrates different detection patterns compared to V3.0, finding ${metrics.totalV30Matches ? ((metrics.totalV30Matches - metrics.totalV32Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}% fewer symptoms but potentially with higher precision.`}
...
async function main() {
  try {
    ...
    console.log(`Detection Difference: ${metrics.totalV32Matches - metrics.totalV30Matches > 0 ? '+' : ''}${metrics.totalV32Matches - metrics.totalV30Matches} (${metrics.totalV30Matches ? ((metrics.totalV32Matches - metrics.totalV30Matches) / metrics.totalV30Matches * 100).toFixed(1) : 'N/A'}%)`);
