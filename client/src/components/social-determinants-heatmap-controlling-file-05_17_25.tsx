...
interface SocialDeterminantsValueInfo {
...
interface HrsnPivotData {
  ...
  maxValue: number;
  detailedData?: Record<string, Record<string, SocialDeterminantsValueInfo>>;
}

// Props definition with optional filters
interface SocialDeterminantsHeatmapProps {
  patientData?: any[];
...
export default function SocialDeterminantsHeatmap({ 
  patientData,
  extractedSymptoms,
  colorScheme = 'vivid', // Default to vivid to match other charts
  isLoading = false,
  filterBy
}: SocialDeterminantsHeatmapProps): JSX.Element {
  ...
  useEffect(() => {
    console.log("SocialDeterminantsHeatmap COMPONENT LOADED at", new Date().toLocaleTimeString());
    console.warn("⚠️ SocialDeterminantsHeatmap WARN - Component is rendering", {
  ...
  // Constants for Social Determinants categories to show in the visualization
  const SOCIAL_DETERMINANTS_CATEGORIES = [
    'housing_insecurity',
    'food_insecurity',
    'transportation',
    'veteran_status',
  ...
  useEffect(() => {
    ...
    console.log("Processing", patientData.length, "patient records for Social Determinants heatmap");
    
    // Create the pivot table structure
    const processedPivotData: HrsnPivotData = {
      rows: SOCIAL_DETERMINANTS_CATEGORIES,
      columns: AGE_RANGES,
      data: {},
      maxValue: 0,
      detailedData: {}
    ...
    SOCIAL_DETERMINANTS_CATEGORIES.forEach(category => {
    ...
    SOCIAL_DETERMINANTS_CATEGORIES.forEach(category => {
      AGE_RANGES.forEach(ageRange => {
        ...
        const detailInfo: SocialDeterminantsValueInfo = {
    ...
    if (categoriesWithDetails.length > 0) {
      ...
      if (ageRangesWithData.length > 0) {
        ...
        console.log("Sample detailed data:", {
          ...
          data: sampleData,
          dataType: "SocialDeterminantsValueInfo"
        });
      }
    }
