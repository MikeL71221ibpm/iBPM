...
import { Maximize2, X, FileText, Code, Table, Printer } from "lucide-react";
...
// Define props for the chart print handler
...
// Simple Chart Print Handler component
function ChartPrintHandler({ chartId, title, onOpenChange, open }: ChartPrintHandlerProps) {
  const printChartRef = React.useRef<HTMLDivElement>(null);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>Print Chart: {title}</DialogTitle>
          <DialogDescription>
            Preview the chart before printing
          </DialogDescription>
        </DialogHeader>
        <div ref={printChartRef} className="h-[400px] overflow-auto border rounded-md p-4">
          <h2 className="text-xl font-semibold mb-2">{title}</h2>
          <div id={`print-chart-${chartId}`} className="h-[350px]">
            {/* Chart content will be inserted here via DOM manipulation */}
...
export default function PopulationHealthCharts({ 
  data, 
  isLoading = false,
  displayMode: externalDisplayMode = "count",
  onDisplayModeChange
  ...
  // Internal display mode state - use this when component is uncontrolled
  const [internalDisplayMode, setInternalDisplayMode] = useState<"count" | "percentage">("count");
  
  // Use external display mode if provided (controlled component), otherwise use internal state
  ...
  const handleDisplayModeChange = (mode: "count" | "percentage") => {
    ...
    // Update internal state if this is an uncontrolled component
    setInternalDisplayMode(mode);
    
    // If we have a parent callback, notify the parent
    ...
    try {
      sessionStorage.setItem('hrsnDisplayMode', mode);
    } catch (e) {
      console.error("Error storing display mode in session storage:", e);
    }
  };
  ...
  useEffect(() => {
    try {
      ...
      // Load display mode - only if not controlled externally
      if (externalDisplayMode === undefined) {
        ...
        if (savedDisplayMode && (savedDisplayMode === "count" || savedDisplayMode === "percentage")) {
          setInternalDisplayMode(savedDisplayMode);
        }
      }
    } catch (err) {
  ...
  const [printingChartId, setPrintingChartId] = useState("");
  ...
  const getFilterVisualizationData = useCallback((filterType: string): { 
    ...
    if (data && data.patients && data.patients.length > 0) {
      ...
      const barChartData = Array.from(aggregatedData.entries()).map(([category, count]) => {
        ...
        return {
          id: category, // Add id to match the expected chart data format
          category: category,
    ...
    if (filterType in emptyCategories) {
      ...
      return {
        barChartData: categories.map((category: string) => ({ 
          id: category, 
          category: category, 
          value: 0,
          rawValue: 0,
  ...
  const getRiskStratificationData = useCallback((): ChartDataItem[] => {
    ...
    const defaultRiskRawData = [
      { id: "High Risk (100+ symptoms)", rawValue: 3 },
      { id: "Medium-High Risk (50-99 symptoms)", rawValue: 5 },
      { id: "Medium Risk (20-49 symptoms)", rawValue: 7 },
      { id: "Low-Medium Risk (10-19 symptoms)", rawValue: 4 },
      { id: "Low Risk (1-9 symptoms)", rawValue: 3 },
      { id: "No Risk (0 symptoms)", rawValue: 2 }
    ];
    
    // Calculate total patients from raw data
    const defaultTotalPatients = defaultRiskRawData.reduce((sum, item) => sum + item.rawValue, 0);
    
    // Calculate percentages based on the total patient count
    const defaultRiskData = defaultRiskRawData.map(item => {
      const percentage = Math.round((item.rawValue / defaultTotalPatients) * 100);
      return {
        ...item,
        percentage,
        // Store both values to avoid NaN during animation
        value: displayMode === "percentage" ? percentage : item.rawValue,
        // Extra properties to help with animation
        countValue: item.rawValue,
  ...
  const getSymptomSegmentData = useCallback((): ChartDataItem[] => {
    ...
    if (data?.symptomSegmentData && data.symptomSegmentData.length > 0) {
      ...
      if (!hasHrsnFactors && (housingFilter !== 'all' || foodFilter !== 'all' || financialFilter !== 'all')) {
        ...
        const enhancedData = data.symptomSegmentData.map((item: any, index: number) => {
          ...
          return {
            ...
            // Store values for both display modes to avoid NaN during animation
            rawValue: item.value,
            percentage: percentage,
      ...
      const filteredData = filterDataByHrsn(nonHrsnItems).map(item => {
        // Get total patients for percentage calculation
        const totalPatients = data?.patients?.length || 24;
        // Calculate percentage based on total patients
        const percentage = Math.round((item.value / totalPatients) * 100);
  ...
  const getDiagnosisData = useCallback((): ChartDataItem[] => {
    ...
    if (data?.diagnosisData && data.diagnosisData.length > 0) {
      ...
      if (!hasHrsnFactors && (housingFilter !== 'all' || foodFilter !== 'all' || financialFilter !== 'all')) {
        ...
        // Add default HRSN factors to all items and calculate percentages
        const totalPatients = data?.patients?.length || 24;
        
        const enhancedData = data.diagnosisData.map((item: any, index: number) => {
          // Calculate percentage based on total patients count
          const percentage = Math.round((item.value / totalPatients) * 100);
      ...
      // Special case for diagnosis data percentages - using a completely new approach
      const totalPatients = data?.patients?.length || 24;
      console.log("Total patients for diagnosis percentage calculation:", totalPatients);
      
      // First, make a copy of the data to avoid modifying the original
      const diagnosisWithPercentages = [];
      
      // Process each diagnosis item individually with explicit percentage calculation
      for (const item of filterDataByHrsn(data.diagnosisData)) {
        // Special safeguard - use explicit values and types
        const count = typeof item.value === 'number' ? item.value : 0;
        // Ensure we're not calculating percentages > 100% per diagnosis
        const safeCount = Math.min(count, totalPatients);
        // Calculate percentage with explicit math
        const percentage = Math.floor((safeCount / totalPatients) * 100);
        
        // Debug each calculation
        console.log(`Diagnosis: ${item.id}, Count: ${count}, Safe Count: ${safeCount}, Total Patients: ${totalPatients}, Percentage: ${percentage}%`);
        
        // Create a new object with the correct structure
        diagnosisWithPercentages.push({
          id: item.id,
          // For count mode
          value: count,
          // For percentage mode - explicitly set
          percentage: percentage,
          // Additional fields to prevent NaN during transitions
          countValue: count,
          percentValue: percentage,
      ...
      // Use the specially processed array
      const processedData = diagnosisWithPercentages;
      
      console.log("After filtering and adding percentages:", processedData.length, "items remain");
      
      // Sort by value in descending order if enabled
      // ALWAYS sort by value in descending order (highest to lowest)
      const sortedData = sortDataDescending(processedData);
  ...
  const getSymptomIDData = useCallback((): ChartDataItem[] => {
    ...
    if (data?.symptomIDData && data.symptomIDData.length > 0) {
      ...
      if (!hasHrsnFactors && (housingFilter !== 'all' || foodFilter !== 'all' || financialFilter !== 'all')) {
        ...
        // Process the data to add percentage values
        const totalPatients = data?.patients?.length || 24;
        
        const processedData = data.symptomIDData.map(item => {
          // Calculate percentage based on total patients count
          const percentage = Math.round((item.value / totalPatients) * 100);
      ...
      // Process the data with percentages
      const totalPatients = data?.patients?.length || 24;
      
      const processedData = filterDataByHrsn(data.symptomIDData).map(item => {
        // Calculate percentage based on total patients count
        const percentage = Math.round((item.value / totalPatients) * 100);
      ...
      console.log("After filtering:", processedData.length, "items remain");
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() => {
                          const chartData = getFullDataset('HRSN Indicators', true);
                          exportToCSV(chartData, 'HRSN_Indicators', toast);
                        }}
                      >
                        <FileText className="h-3.5 w-3.5" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Export as CSV (basic data)</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() => {
                          const chartData = getFullDataset('HRSN Indicators', true);
                          exportToDetailedCSV(chartData, 'HRSN_Indicators_Detailed', toast, {
                            displayMode: displayMode,
                            patientCount: data?.patients?.length || 24,
                            recordCount: data?.totalRecords || 1061
                          });
                        }}
                      >
                    ...
                    <TooltipContent>Export as Detailed CSV (includes metadata)</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() => {
                          const chartData = getFullDataset('HRSN Indicators', true);
                          exportToExcel(chartData, 'HRSN_Indicators', toast, {
                            displayMode: displayMode,
                            patientCount: data?.patients?.length || 24,
                            recordCount: data?.totalRecords || 1061
                          });
                    ...
                    <TooltipContent>Export as Excel (formatted spreadsheet)</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() => {
                          const chartData = getFullDataset('HRSN Indicators', true);
                          exportToJSON(chartData, 'HRSN_Indicators', toast);
                        }}
                      >
                        <FileJson className="h-3.5 w-3.5" />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>Export as JSON (for developers)</TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        className="h-6 w-6 p-0"
                        onClick={() => {
                          // Open a printable version of the chart
                          setPrintingChartId('HRSN_Indicators');
                    ...
                    <TooltipContent>Print current view</TooltipContent>
                  </Tooltip>
                </div>
              </div>
            ...
            <CardContent className="p-2 h-[280px]">
              <ResponsiveBar
                data={getHrsnIndicatorData().map(item => {
                  // Pre-calculate all needed values to avoid NaN during animations
                  const percentage = item.percentage || 0;
                  const displayValue = displayMode === "percentage" ? percentage : (item.value || 0);
                  ...
                  return {
                    ...
                    percentage: percentage,
                    // Store both values to avoid animation issues
                    countValue: item.value || 0,
                    percentValue: percentage
                  };
                })}
                keys={['value']}
                ...
                enableLabel={true}
                label={d => { 
                  // Use pre-calculated values to avoid NaN
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  ...
                  <ChartExportButtons
                    ...
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('HRSN Indicators', true);
                      exportToDetailedCSV(chartData, 'HRSN_Indicators', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
            ...
            <CardContent className="p-2 h-[280px]">
              <ResponsiveBar
                data={getRiskStratificationData().map(item => ({
                  ...item,
                  // Calculate percentages correctly by dividing by total patients
                  value: displayMode === "percentage" 
                    ? Math.min(Math.round((item.value / (data?.patients?.length || 24)) * 100), 100)
                    : item.value
                }))}
                keys={['value']}
                indexBy="id"
                ...
                enableLabel={true}
                label={d => {
                  // Use countValue or percentValue to avoid NaN during animation
                  return displayMode === "percentage" 
                    ? `${d.data.percentValue || d.data.percentage || 0}%` 
                    : `${d.data.countValue || d.data.rawValue || d.value || 0}`;
                }}
                labelTextColor={"#000000"}
                labelPosition="middle"
                labelOffset={-3}
            ...
            <CardFooter className="p-2 flex justify-between">
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  ...
                  <ChartExportButtons
                    onExportCSV={() => {
                      const chartData = getFullDataset('Risk Stratification', true);
                      exportToCSV(chartData, 'Risk_Stratification', toast);
                    }}
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('Risk Stratification', true);
                      exportToDetailedCSV(chartData, 'Risk_Stratification', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportExcel={() => {
                      const chartData = getFullDataset('Risk Stratification', true);
                      exportToExcel(chartData, 'Risk_Stratification', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportJSON={() => {
                      const chartData = getFullDataset('Risk Stratification', true);
                      exportToJSON(chartData, 'Risk_Stratification', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
            ...
            <CardFooter className="p-2 flex justify-between">
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  ...
                  <ChartExportButtons
                    onExportCSV={() => {
                      const chartData = getFullDataset('Symptom Segment', true);
                      exportToCSV(chartData, 'Symptom_Segment', toast);
                    }}
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('Symptom Segment', true);
                      exportToDetailedCSV(chartData, 'Symptom_Segment', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportExcel={() => {
                      const chartData = getFullDataset('Symptom Segment', true);
                      exportToExcel(chartData, 'Symptom_Segment', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportJSON={() => {
                      const chartData = getFullDataset('Symptom Segment', true);
                      exportToJSON(chartData, 'Symptom_Segment', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
            ...
            <CardContent className="p-2 h-[280px]">
              <ResponsiveBar
                data={getDiagnosisData().map(item => ({
                  ...item,
                  // Set the appropriate value based on display mode with proper percentage calculation
                  value: displayMode === "percentage" 
                    ? Math.min(Math.round((item.value / (data?.patients?.length || 24)) * 100), 100)
                    : item.value
                }))}
            ...
            <CardFooter className="p-2 flex justify-between">
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  ...
                  <ChartExportButtons
                    onExportCSV={() => {
                      const chartData = getFullDataset('Diagnosis', true);
                      exportToCSV(chartData, 'Diagnosis', toast);
                    }}
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('Diagnosis', true);
                      exportToDetailedCSV(chartData, 'Diagnosis', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportExcel={() => {
                      const chartData = getFullDataset('Diagnosis', true);
                      exportToExcel(chartData, 'Diagnosis', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportJSON={() => {
                      const chartData = getFullDataset('Diagnosis', true);
                      exportToJSON(chartData, 'Diagnosis', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Showing symptom IDs from a total of {data?.patients?.length || 24} patients.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <ChartExportButtons
                    onExportCSV={() => {
                      const chartData = getFullDataset('Symptom ID', true);
                      exportToCSV(chartData, 'Symptom_ID', toast);
                    }}
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('Symptom ID', true);
                      exportToDetailedCSV(chartData, 'Symptom_ID', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportExcel={() => {
                      const chartData = getFullDataset('Symptom ID', true);
                      exportToExcel(chartData, 'Symptom_ID', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportJSON={() => {
                      const chartData = getFullDataset('Symptom ID', true);
                      exportToJSON(chartData, 'Symptom_ID', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
              ...
              <Dialog>
                ...
                <DialogContent className="max-w-4xl h-[80vh]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Showing diagnostic categories from a total of {data?.patients?.length || 24} patients.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <ChartExportButtons
                    onExportCSV={() => {
                      const chartData = getFullDataset('Diagnostic Category', true);
                      exportToCSV(chartData, 'Diagnostic_Category', toast);
                    }}
                    onExportDetailCSV={() => {
                      const chartData = getFullDataset('Diagnostic Category', true);
                      exportToDetailedCSV(chartData, 'Diagnostic_Category', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportExcel={() => {
                      const chartData = getFullDataset('Diagnostic Category', true);
                      exportToExcel(chartData, 'Diagnostic_Category', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                    ...
                    onExportJSON={() => {
                      const chartData = getFullDataset('Diagnostic Category', true);
                      exportToJSON(chartData, 'Diagnostic_Category', toast, {
                        displayMode: displayMode,
                        patientCount: data?.patients?.length || 24,
                        recordCount: data?.totalRecords || 1061
                      });
                              ...
                              return (
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                      ...
                                      <Card className="shadow-md">
                                        ...
                                        <CardContent>
                                          <CategoricalHrsnChart 
                                            data={data?.patients || []}
                                            title="HRSN Categories Distribution"
                                      ...
                                      <Card className="shadow-md">
                                        ...
                                        <CardContent>
                                          {console.log("🔵🔵🔵 GENDER CARD RENDERING ATTEMPT - About to render CategoricalHrsnChart")}
                                          <CategoricalHrsnChart
                                      ...
                                      <Card className="shadow-md">
                                        ...
                                        <CardContent>
                                          <CategoricalHrsnChart
                                            ...
                                            title="Financial Status Distribution"
                                            filterBy={{
                                              diagnosis: diagnosisFilter !== "all" ? diagnosisFilter : undefined
                                            }}
                                          />
