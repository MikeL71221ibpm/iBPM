...
// Import from controlling file with today's date
import PopulationHealthCharts from "@/components/population-health-charts-controlling-file-05_12_25"; // May 12, 2025
...
export default function PopulationSearch() {
    ...
    try {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          // Use V3.3.2 parallel extraction for efficient processing
          console.log("🔍 Starting V3.3.2 parallel symptom extraction");
          
          const extractionResponse = await apiRequest("POST", "/api/v3.3.2/extract-symptoms-parallel", {
            patientIds: searchData.patients.map((patient: any) => patient.patientId),
            boostMode: true // Enable boost mode for faster processing
          });
          
          if (!extractionResponse.ok) {
            throw new Error("V3.3.2 parallel extraction failed");
          }
          
          const extractionData = await extractionResponse.json();
          console.log(`✅ V3.3.2 extraction completed: ${extractionData.symptomsExtracted} symptoms from ${extractionData.notesProcessed} notes`);
          
          // Now get visualization data with updated symptom data
    ...
    if (!autoLoadedData && !apiData) {
      ...
      (async () => {
        try {
          ...
          const vizResponse = await fetch("/api/visualization-data", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              patientIds: patientIds,
          ...
          const vizData = await vizResponse.json();
          console.log("🔄 AUTO: Visualization data received");
  ...
  }, [autoLoadedData, apiData, setCurrentData]);
  ...
  return (
    <div className="space-y-6">
      ...
      {isRunningAnalysis && !isAnalyzing ? (
        ...
        <>
          {/* Only show charts when not analyzing - direct component with no wrapper */}
