...
interface RaceInfographicProps {
  title: string;
...
export function RaceInfographic({ 
  title, 
  totalPatients, 
  patientData,
  categoryName 
}: RaceInfographicProps) {
  
  // Process patient data to find top ZIP codes and their predominant race
  const processedData = useMemo(() => {
    ...
    const zipGroups = patientData.reduce((acc: any, patient: any) => {
      ...
      // Count races within this ZIP code
      const race = patient.race || 'Unknown';
      acc[zip].raceCounts[race] = (acc[zip].raceCounts[race] || 0) + 1;
      
      return acc;
    }, {});
    
    // Convert to array and find predominant race for each ZIP
    const zipData = Object.values(zipGroups).map((group: any) => {
      // Find predominant race
      let predominantRace = 'Unknown';
      let maxCount = 0;
      
      Object.entries(group.raceCounts).forEach(([race, count]: [string, any]) => {
        if (count > maxCount) {
          maxCount = count;
