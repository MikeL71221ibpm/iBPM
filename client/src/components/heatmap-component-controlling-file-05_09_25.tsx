...
const HeatmapComponent = ({ title, data, indexBy }: HeatmapProps) => {
  ...
  const groupedData = data.reduce((acc, item) => {
    ...
    const dateStr = new Date((item as any).dos_date || (item as any).dosDate).toLocaleDateString();
  ...
  const allDates = [...new Set(data.map(item => 
    new Date((item as any).dos_date || (item as any).dosDate).toLocaleDateString()
  ...
  // Colors for heatmap
  const colors = ["#FFFFFF", "#FFFFFF", "#B3CDE3", "#F2DD82", "#FDAE6B", "#FF0000"];
