...
// Enhancement: Added TypeScript fixes and better data typing
    ...
    const fetchPatientData = async () => {
      try {
        ...
        if (result.heatmapData) {
          ...
          let max = 0;
          const heatmapValues = result.heatmapData.values || {};
          
          Object.keys(heatmapValues).forEach(row => {
            if (heatmapValues[row]) {
              Object.keys(heatmapValues[row]).forEach(col => {
                const value = heatmapValues[row][col] || 0;
        ...
        setError(err instanceof Error ? err.message : "Failed to load HRSN data");
        setData(null);
      } finally {
        setLoading(false);
      }
