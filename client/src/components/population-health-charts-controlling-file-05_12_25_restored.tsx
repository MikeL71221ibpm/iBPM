...
export default function PopulationHealthCharts({ 
  ...
  const handleDisplayModeChange = (mode: "count" | "percentage") => {
    ...
    // Redirect to appropriate page based on selected mode
    if (mode === "percentage") {
      window.location.href = '/population-health-percentage';
    } else {
      window.location.href = '/population-health';
    }
  };
  
  // Track last update time to force rerenders when needed
  ...
  // Save display mode to localStorage for persistence and handle data updates
  useEffect(() => {
    try {
      localStorage.setItem('chartDisplayMode', displayMode);
      console.log("Display mode changed to:", displayMode);
      
      // Force data recalculation after display mode change
      const totalPatients = data?.patients?.length || 24;
      console.log("Data will be recalculated for", totalPatients, "patients in", displayMode, "mode");
      
      // Add this console log to help debug percentage data issues
      if (displayMode === "percentage") {
        const riskData = getRiskStratificationData();
        console.log("Percentage mode - Risk data sample:", riskData.slice(0, 2));
      }
    } catch (err) {
      console.error("Error handling display mode change:", err);
    }
  }, [displayMode, lastUpdate]);
    ...
    if (data && data.patients && data.patients.length > 0) {
      ...
      const barChartData = Array.from(aggregatedData.entries()).map(([category, count]) => {
        ...
        return {
          ...
          id: category // Add id field to match ChartDataItem interface
        }
      });
  ...
  const getRiskStratificationData = useCallback((): ChartDataItem[] => {
    console.log("getRiskStratificationData called for " + displayMode + " view");
    ...
    // If in percentage mode, update the value field directly
    if (displayMode === "percentage") {
      defaultRiskData = defaultRiskData.map(item => ({
        ...item,
    ...
    if (data?.data && Array.isArray(data.data) && data.data.length > 0 && data?.patients) {
      ...
      return riskLevels.map((level: { id: string, min: number, max: number }) => ({
        ...
        value: 0,
        rawValue: 0,
        percentage: 0 // Zero percentage since no patients match these criteria
      }));
    }
  ...
  const getFullDataset = (chartType: string, includePatientData: boolean = false): ChartDataItem[] => {
    ...
    switch (chartType) {
      case 'HRSN Indicators':
        ...
        dataSet = [...hrsnData].sort((a, b) => (b.value || 0) - (a.value || 0));
        break;
      ...
      case 'Symptom Segment':
        ...
        if (data?.symptomSegmentData && data.symptomSegmentData.length > 0) {
          ...
          dataSet = filterDataByHrsn(nonHrsnItems).sort((a, b) => (b.value || 0) - (a.value || 0));
        }
        break;
      ...
      case 'Diagnostic Category':
        if (data?.diagnosticCategoryData && data.diagnosticCategoryData.length > 0) {
          dataSet = filterDataByHrsn(data.diagnosticCategoryData).sort((a, b) => (b.value || 0) - (a.value || 0));
        }
        break;
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            {/* Count/Percentage Button */}
            <Button 
              variant="outline" 
              className="flex items-center gap-2"
              onClick={() => window.location.href = '/population-health-percentage'}
            >
            ...
            <CardContent className="p-2 h-[280px]">
              <ResponsiveBar
                data={getHrsnIndicatorData().map(item => ({
                  ...item,
                  // Use the appropriate value based on displayMode, ensuring we never pass undefined/NaN
