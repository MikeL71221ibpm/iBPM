...
import BubbleChartFixed from "@/components/BubbleChartFixed";
import BubbleChartRevised from "@/components/BubbleChartRevised";
...
const HeatmapVisualization = ({ data, title }: { data: PivotData | undefined, title: string }) => {
  ...
  // CSS for the sticky footer at the bottom of the table
  const stickyFooterStyle = `
    ...
    /* Force column widths to be exactly the same */
    table {
  ...
  return (
    <Card className="mb-4">
      ...
      <CardContent className="px-2">
        ...
        <div className="bg-white rounded" ref={containerRef}>
          ...
          {/* Table container with fixed height to ensure scrollable area */}
          <div className="relative" style={{ height: '450px' }}>
            {/* This container is scrollable */}
            <div 
              className="h-full overflow-x-auto overflow-y-auto relative transition-transform" 
              style={{ 
                transform: `scale(${scaleFactor})`, 
                transformOrigin: 'top left',
              }}
            >
              <table className="min-w-full border-collapse w-auto" style={{ tableLayout: 'fixed', borderSpacing: 0 }}>
                <colgroup>
                  ...
                  {topRowsWithRanks.map(({row, originalRow, rank, sum}) => (
                    <tr key={row} className="border-b border-gray-200">
                      ...
                      {data.columns.map(column => {
                        // Use originalRow to access data, but display with the clean row
                ...
                <tfoot className="sticky-footer">
                  <tr>
                    ...
                    {data.columns.map(column => (
                      <th 
                        key={`date-${column}`} 
                        className="date-header bg-white border-r border-gray-100"
                        style={{ width: '40px' }}>
                        <div className="date-container">
                          <div className="date-label font-semibold text-gray-800">
                            {column}
                          </div>
                        </div>
                      </th>
