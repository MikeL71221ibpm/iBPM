...
const VisualizationCard = ({ 
  ...
  const renderHeatmap = () => {
    ...
    if (!Array.isArray(data.rows) || data.rows.length === 0) {
      return <NoDataDisplay message="No items available for heatmap visualization" />;
    }
    
    if (!Array.isArray(data.columns) || data.columns.length === 0) {
      return <NoDataDisplay message="No date columns available for heatmap visualization" />;
    }
    
    // Check if data structure is complete
    if (!data.data || typeof data.data !== 'object') {
      return <NoDataDisplay message="Invalid data structure for heatmap" />;
    }
...
export default function AutoPivotHeatmaps() {
