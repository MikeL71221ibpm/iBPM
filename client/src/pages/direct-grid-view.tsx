...
const HardCodedPivot = ({ data, dataType, compact = true }: { 
  ...
  useEffect(() => {
    ...
    styleEl.textContent = `
      ...
      .date-cell {
        ...
        width: ${compact ? '30px' : '40px'} !important;
        padding: 0 !important;
        border-right: 1px solid #e5e7eb !important;
        background-color: white !important;
        height: ${compact ? '30px' : '40px'} !important;
      }
    ...
    // Always force scrollbars to be visible
    container.style.overflowY = 'scroll'; // Use 'scroll' instead of 'auto' to always show scrollbars
    ...
    legendTitle.style.fontSize = compact ? '10px' : '12px';
    legendContainer.appendChild(legendTitle);
    
    // Create dynamic legend items based on data thresholds with Iridis colors
    const legendItems = [
      { level: 'level-0', text: '0 mentions', color: 'white' },
      { level: 'level-1', text: `1-${thresholdLevel2-1} mentions`, color: '#EDF8FB' },
      { level: 'level-2', text: `${thresholdLevel2}-${thresholdLevel3-1} mentions`, color: '#B3CDE3' },
      { level: 'level-3', text: `${thresholdLevel3}-${thresholdLevel4-1} mentions`, color: '#8C96C6' },
      { level: 'level-4', text: `${thresholdLevel4}-${thresholdLevel5-1} mentions`, color: '#8856A7' },
      { level: 'level-5', text: `${thresholdLevel5}+ mentions`, color: '#994C99' }
    ];
    ...
    legendItems.forEach(item => {
      ...
      itemText.style.fontSize = compact ? '9px' : '11px';
    ...
    // Add download buttons to right side
    ...
    topRows.forEach(({row, sum}, rowIndex) => {
      ...
      data.columns.forEach(column => {
        ...
        // Assign color class based on fixed value thresholds (like in the backup version)
        if (value === 0) {
          td.className = 'data-cell level-0';
        } else if (value >= 20) { // Fixed thresholds
    ...
    emptyCell.style.fontSize = compact ? '10px' : '12px';
    ...
    data.columns.forEach((column) => {
      ...
      dateLabel.style.fontSize = compact ? '7px' : '9px';
      ...
      <div className="p-2">
        {isLoading ? (
          ...
          <div className="mt-1" style={{ 
            // Dynamic height based on data type - smaller for HRSN with fewer rows
          ...
          <div className="flex justify-between mb-4">
            <div>
              <Select 
                ...
                onValueChange={(value) => {
                  // Update sessionStorage with the selected patient ID
                  sessionStorage.setItem('selectedPatientId', value);
                  
                  // Store the patient name in sessionStorage
                  const patientName = `Bob Test${value}`;
                  sessionStorage.setItem('selectedPatientName', patientName);
                  
                  // Update the URL to reflect the new patient
                  setLocation(`/direct-grid-view/${value}`);
                }}
              >
            ...
            <div className="flex gap-2">
              <Button 
                variant="outline" 
                onClick={() => {
                  // Ensure we have the correct patient ID in sessionStorage
                  sessionStorage.setItem('selectedPatientId', patientToDisplay);
