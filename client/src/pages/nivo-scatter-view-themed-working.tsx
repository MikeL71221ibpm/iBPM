...
const ScatterSection = ({ 
  ...
  const downloadAsPNG = useCallback(async () => {
    ...
    try {
      ...
      // Create a temporary wrapper element for export
      ...
      // Create patient info header that will always be included
      const patientName = getPatientName(parseInt(patientId));
      
      const patientInfoHeader = document.createElement('div');
      patientInfoHeader.style.backgroundColor = '#f8fafc';
      patientInfoHeader.style.padding = '15px';
      patientInfoHeader.style.borderRadius = '4px';
      patientInfoHeader.style.border = '1px solid #e5e7eb';
      patientInfoHeader.style.marginBottom = '15px';
      ...
      // For dialog mode, make sure we get the inner div with the actual chart
      if (dialogOpen && isFullPageView) {
        // Find the inner chart div in full page mode
        const innerDiv = chartElement.querySelector('.flex-1') || chartElement;
        tempWrapper.appendChild(innerDiv.cloneNode(true));
      } else {
        // For regular view or standard dialog view
        tempWrapper.appendChild(chartElement);
      }
      
      // Append to the document to render
  ...
  }, [chartRef, dataType, patientId, dialogOpen, displayName, isFullPageView, toast]);
