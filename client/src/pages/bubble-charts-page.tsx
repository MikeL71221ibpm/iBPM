...
import { Loader2, FileSpreadsheet, FileImage, File, TableProperties, Grid3X3, RotateCw, Maximize2, X } from 'lucide-react';
...
// Color themes for the bubble charts
const COLOR_THEMES = {
  // Enhanced Iridis (purple-blue) theme with accessibility improvements
  iridis: {
    name: "Iridis (Purple-Blue)",
    HIGHEST: '#4B0082',  // Highest - Deep indigo for better contrast
    HIGH: '#6A0DAD',     // High - Vibrant deep purple 
    MEDIUM: '#9370DB',   // Medium - Medium purple - clearly different 
    LOW: '#B19CD9',      // Low - Light purple - clearly different
    LOWEST: '#DDA0DD',   // Lowest - Plum color - better visibility than near-white
  },
  ...
  // Enhanced Plasma color scheme with more contrast
  plasma: {
    name: "Plasma (High-contrast)",
    HIGHEST: '#0D0887',  // Highest - Deep blue
    HIGH: '#7E03A8',     // High - Deep purple - more distinct
    MEDIUM: '#CC4678',   // Medium - Pink - clearly different
    LOW: '#F0F921',      // Low - Yellow - more distinct
    LOWEST: '#FCFFA4',   // Lowest - Light yellow
  },
  
  // Red-Blue diverging color scheme
  redBlue: {
    name: "Red-Blue",
    HIGHEST: '#9E0142',  // Highest - Bright red
    HIGH: '#F46D43',     // High - Orange/salmon
    MEDIUM: '#FFFFFF',   // Medium - White
    LOW: '#74ADD1',      // Low - Light blue
    LOWEST: '#313695',   // Lowest - Dark blue
  },
  
  // Grayscale theme for printing
  grayscale: {
    name: "Grayscale",
    HIGHEST: '#000000',  // Highest - Black
    HIGH: '#444444',     // High - Dark gray
    MEDIUM: '#888888',   // Medium - Medium gray
    LOW: '#CCCCCC',      // Low - Light gray
    LOWEST: '#F0F0F0',   // Lowest - Very light gray
  },
};
...
// DEPRECATED: Custom color themes - REPLACED by centralized chart-theme-system.ts
// This section will be removed as part of platform standardization
...
const ScatterSection = ({ 
  ...
  const scatterData = React.useMemo(() => {
      ...
      sortedRows.forEach((row, rowIndex) => {
        ...
        data.columns.forEach((column) => {
          ...
          if (value > 0) {
            ...
            // Debug logging for specific disorder
            if (row.includes("Persistent Depressive")) {
              console.log(`🔍 PERSISTENT DEPRESSIVE DEBUG:`, {
                row: row,
                rowWithCount: rowWithCount,
                column: column,
                value: value,
                sessionsCount: sessionsCount,
                dataPoint: { x: String(column), y: rowWithCount, size: value, frequency: sessionsCount }
              });
            }
          }
        });
    ...
    // Debug specific disorder in final series
    const persistentSeries = series.find(s => s.id.includes("Persistent Depressive"));
    if (persistentSeries) {
      console.log(`🎯 PERSISTENT DEPRESSIVE FINAL SERIES:`, persistentSeries);
    } else {
      console.log(`❌ PERSISTENT DEPRESSIVE NOT FOUND in final series. Available series:`, series.map(s => s.id));
    }
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-4xl h-[80vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Detailed view of {displayName.toLowerCase()} data for this patient
            </DialogDescription>
          </DialogHeader>
          
          {/* Export Widget in Dialog */}
          {showExportWidget && (
            <ChartExportWidget
              ...
              chartId={`scatter-${patientId}-${dataType}`}
          ...
          <div 
            className="h-[calc(80vh-120px)] overflow-auto"
            ref={chartRef}
          >
            {data && (
              <div style={{ height: '600px', width: '100%' }}>
                <ScatterVisualization 
                  ...
                  rowColors={rowColors}
                  dataType={dataType}
                  onBubbleClick={() => setDialogOpen(true)}
                />
              </div>
            )}
          </div>
      ...
      <div className="p-2">
        {isLoading ? (
          ...
          <div 
            className="mt-1" 
            style={{ 
              height: dataType === 'hrsn' ? '300px' : '280px', // Restore HRSN proper height, others compact
              overflowY: dataType === 'hrsn' ? 'scroll' : 'auto', // HRSN scroll behavior
              overflowX: 'auto',
          ...
          >
            <div style={{
              height: dataType === 'hrsn' ? 
                Math.max(25 * (data?.rows.length || 0), 400) + 'px' : 
                '280px', // Restore HRSN original dynamic height, keep others compact
              width: '100%'
            }}>
              <ScatterVisualization 
                data={scatterData} 
...
const ScatterVisualization = ({ 
  ...
  // Add toast hook for onClick notifications
  ...
  const lastTooltipCloseTime = React.useRef<number>(0);
  ...
  // Calculate margins based on the length of the row labels - increased for better Y-axis visibility
  const maxLabelLength = Math.max(...rows.map(row => row.length));
  const leftMargin = Math.min(Math.max(8 * maxLabelLength, 150), compact ? 300 : 400);
  
  // Margins and styling - different bottom margins for HRSN vs other charts
  const margin = compact 
    ? { 
        top: 25, 
        right: 20, 
        bottom: dataType === 'hrsn' ? 50 : 70, // More space for non-HRSN X-axis labels
        left: leftMargin 
      } 
    : { top: 40, right: 40, bottom: 60, left: leftMargin };
...
const processedData = data.map(series => {
  ...
  return (
    ...
    >
            ...
            <div className="flex justify-between items-center mb-1 pb-1 border-b">
              ...
              <button 
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  // Clear all timers when manually closing
                  if (tooltipTimerRef.current !== null) {
                    window.clearTimeout(tooltipTimerRef.current);
                    tooltipTimerRef.current = null;
                  }
                  ...
                  // Record the time when tooltip was manually closed
                  lastTooltipCloseTime.current = Date.now();
      ...
      <div style={{ 
        height: (() => {
          const itemCount = processedData[0]?.data.length || 0;
          const isHrsn = dataType === 'hrsn';
          if (isHrsn && itemCount <= 6) {
            // For HRSN charts with few items, use much smaller height for tighter spacing
            const tightHeight = Math.max(150, itemCount * 50); // Minimum 150px, 50px per item
            console.log(`HRSN tight spacing: ${itemCount} items, height: ${tightHeight}px`);
            return tightHeight;
          }
        ...
        // Size based on standardized 10-tier bubble scale from utility function - maintaining original ratios
        nodeSize={(d: any) => {
          console.log("Calculating node size for data:", d?.data);
          if (!d?.data) return 8; // Keep original default size
          ...
          // Use the utility function to calculate the bubble size - keeping original ratios
          const baseSize = calculateBubbleSize(value);
          
          // Keep original sizing to maintain proper ratios between bubbles
          // Accessibility will be handled through larger hover detection area
          const finalSize = Math.max(baseSize, 8); // Minimum 8px as originally designed
          
          // Log the value and resulting size to help debugging
          console.log(`Node with frequency ${value} gets size ${finalSize} from standardized bubble size utility (base: ${baseSize})`);
          
          return finalSize;
        }}
        // Custom layer to render larger invisible hover areas for accessibility
        layers={[
          'grid', 'axes', 'nodes', 'markers', 'mesh', 'legends',
          // Add custom hover area layer
          (props: any) => {
            return (
              <g>
                {props.nodes.map((node: any) => (
                  <circle
                    ...
                    cx={node.x}
                    cy={node.y}
                    r={12} // Further reduced to 12px for even more precise hover control
                    fill="transparent"
                    style={{ 
                      pointerEvents: 'all',
                      cursor: 'pointer'
                    ...
                    onMouseEnter={(e) => {
                      ...
                      // Set tooltip data with 50ms delay for faster response
                      if (tooltipTimerRef.current !== null) {
                        window.clearTimeout(tooltipTimerRef.current);
                      }
                      
                      tooltipTimerRef.current = window.setTimeout(() => {
                        // Check if enough time has passed since last manual close
                        const timeSinceClose = Date.now() - lastTooltipCloseTime.current;
                        const minimumDelay = timeSinceClose < 1000 ? 1000 - timeSinceClose : 0;
                        
                        if (minimumDelay > 0) {
                          // Wait additional time if tooltip was recently closed
                          tooltipTimerRef.current = window.setTimeout(() => {
                            setTooltipData({
                              x: node.data.x || 'Unknown',
                              y: node.data.y || 'Unknown', 
                              size: node.data.size || 0,
                              frequency: node.data.frequency || 0,
                              visible: true,
                              color: rowColors[node.serieId] || '#3B82F6'
                            });
                            tooltipTimerRef.current = null;
                          }, minimumDelay);
                        } else {
                      ...
                      }, 30); // Reduced delay for even faster tooltip appearance
                    }}
                    onMouseLeave={(e) => {
                      // Clear the show timer if still pending
                      if (tooltipTimerRef.current !== null) {
                        window.clearTimeout(tooltipTimerRef.current);
                        tooltipTimerRef.current = null;
                      }
                      
                      // Immediate tooltip dismissal - no delay for faster response
                      setTooltipData(null);
                    }}
                    onClick={(e) => {
                      if (props.onClick) {
        ...
        colors={(d: any) => {
          ...
          // CRITICAL FIX: Check if we have a color for this exact key
          let color = rowColors[key];
          
          // If no color found, assign a visible fallback color instead of gray
          if (!color) {
            // Use a bright, visible color as fallback instead of invisible gray
            color = '#3B82F6'; // Bright blue as fallback
            
            // Log the issue for debugging
            console.log(`⚠️ NO COLOR FOUND for key "${key}" - using blue fallback. Available rowColors keys:`, Object.keys(rowColors));
          }
          
          // Debug specific disorder
          if (key && key.includes("Persistent Depressive")) {
            console.log(`🔍 PERSISTENT DEPRESSIVE COLOR DEBUG:`, {
              key: key,
              assignedColor: color,
              hasColorInMap: !!rowColors[key],
              rowColorsKeys: Object.keys(rowColors)
            });
          }
        ...
        onMouseMove={(node: any, event) => {
          ...
          // Set a new timer to show the tooltip with smart delay
          tooltipTimerRef.current = window.setTimeout(() => {
            if (node && node.data) {
              // Check if enough time has passed since last manual close
              const timeSinceClose = Date.now() - lastTooltipCloseTime.current;
          ...
          }, 50); // Initial delay still fast for normal hovering
        }}
        onMouseLeave={() => {
          ...
          // Keep tooltip visible for at least 3 seconds for accessibility
          if (tooltipData?.visible) {
            tooltipCleanupRef.current = window.setTimeout(() => {
              setTooltipData(null);
              tooltipCleanupRef.current = null;
            }, 3000); // 3 seconds minimum visibility for accessibility
          }
        }}
        onClick={(node: any) => {
          // Enhanced onClick handler for bubble chart nodes
          if (node && node.data) {
            const datum = node.data;
            const itemName = datum.y || 'Unknown item';
            const sessionDate = datum.x || 'Unknown date';
            const occurrenceCount = datum.size || 0;
            const frequencyCount = datum.frequency || 1;
            
            // Get the color for this row from the series ID
            const seriesId = node.serieId || '';
            const color = rowColors[seriesId] || '#3B82F6';
            
            // Show detailed toast notification with longer duration
            toast({
              title: `📊 ${itemName}`,
              description: (
                <div className="space-y-1">
                  <p><strong>Session:</strong> {sessionDate}</p>
                  <p><strong>Mentions in this session:</strong> {occurrenceCount}</p>
                  <p><strong>Total sessions with this item:</strong> {frequencyCount}</p>
                  {occurrenceCount > 1 && (
                    <p className="text-amber-600 text-sm">⚠️ Multiple mentions in single session</p>
                  )}
                </div>
              ),
              duration: 8000, // 8 seconds for better visibility
              style: {
            ...
            // Also hide any existing tooltip to prevent overlap and record close time
            lastTooltipCloseTime.current = Date.now();
            setTooltipData(null);
            
            // Open individual chart dialog when clicking on bubble
            if (onBubbleClick) {
              onBubbleClick();
            }
...
function NivoScatterViewThemed() {
  ...
  const [showExpandedExportWidget, setShowExpandedExportWidget] = React.useState(true);
  
  // State for theme selection - default to 'iridis'
  const [currentTheme, setCurrentTheme] = React.useState('iridis');
  
  // State for paginated chart expansion
  ...
  // Reset export widget visibility when expanded chart changes
  React.useEffect(() => {
    if (expandedChart) {
      setShowExpandedExportWidget(true);
    }
  }, [expandedChart]);
  ...
  const exportGridAsPDF = useCallback(async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(gridRef.current, {
        ...
        backgroundColor: '#fff',
        logging: false,
        windowWidth: gridRef.current?.scrollWidth || 1200,
        windowHeight: gridRef.current?.scrollHeight || 800,
        width: gridRef.current?.scrollWidth || 1200,
        height: gridRef.current?.scrollHeight || 800
      });
  ...
  return (
    <div className="container mx-auto py-4 px-12">
      {/* Full Database Stats Widget */}
      <DatabaseStatsWidget />
      
      <h1 className="text-2xl font-bold mb-4">Chart Analysis</h1>
      <div className="flex flex-col space-y-4">
        ...
        {/* Display all four scatter plots in a 2x2 grid - compact layout like HRSN Indicators */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2" ref={gridRef}>
          {DATA_TYPES.map((type) => (
            <ScatterSection 
              key={type.id} 
              dataType={type.id} 
      ...
      {/* Paginated Chart Modal */}
      {expandedChart && (
        <Dialog open={!!expandedChart} onOpenChange={() => setExpandedChart(null)}>
          <DialogContent className="w-[95vw] max-w-[95vw] h-[90vh] max-h-[90vh] p-0 m-0">
            <div className="flex flex-col h-full">
                ...
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1">
                    <DialogTitle className="text-xl font-bold">
                      {expandedChart.title} - Block View
                    </DialogTitle>
                    <DialogDescription>
                      Showing rows {((expandedChart.currentPage - 1) * ROWS_PER_PAGE) + 1}-{Math.min(expandedChart.currentPage * ROWS_PER_PAGE, expandedChart.rows.length)} of {expandedChart.rows.length} ({expandedChart.totalPages} blocks of {ROWS_PER_PAGE} rows each)
                    </DialogDescription>
                  </div>
                  
                  <div className="flex items-center gap-3">
                    {/* Pagination Controls */}
                    {expandedChart.totalPages > 1 && (
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setExpandedChart(prev => prev ? {...prev, currentPage: Math.max(1, prev.currentPage - 1)} : null)}
                          disabled={expandedChart.currentPage === 1}
                        >
                          Previous Block
                        </Button>
                        <span className="text-sm text-muted-foreground">
                          Block {expandedChart.currentPage} of {expandedChart.totalPages}
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => setExpandedChart(prev => prev ? {...prev, currentPage: Math.min(prev.totalPages, prev.currentPage + 1)} : null)}
                          disabled={expandedChart.currentPage === expandedChart.totalPages}
                        >
                          Next Block
                        </Button>
                      </div>
              ...
              {/* Paginated chart container */}
              <div className="relative flex-1 p-6 bg-gradient-to-br from-slate-50 to-white">
                <div className="w-full h-full bg-white rounded-lg shadow-sm border border-slate-200 p-4">
                  <div style={{ width: '100%', height: '100%' }}>
                    <ScatterVisualization 
                      data={(() => {
                        // Calculate which rows to show for current page
                        const startIndex = (expandedChart.currentPage - 1) * ROWS_PER_PAGE;
                        const endIndex = startIndex + ROWS_PER_PAGE;
                        const pageRows = expandedChart.rows.slice(startIndex, endIndex);
                        
                        // Filter scatter data to only include points for visible rows
                        return expandedChart.scatterData.map((series: any) => ({
                          ...series,
                          data: series.data.filter((point: any) => {
                            // More precise filtering - match exact row or row without count
                            const pointRow = point.y;
                            return pageRows.some(row => {
                              // Exact match
                              if (pointRow === row) return true;
                              // Match without count suffix (e.g., "item (1)" matches "item")
                              const rowWithoutCount = row.replace(/ \(\d+\)$/, '');
                              const pointWithoutCount = pointRow.replace(/ \(\d+\)$/, '');
                              return rowWithoutCount === pointWithoutCount;
                            });
                          })
                        })).filter(series => series.data.length > 0); // Remove empty series
                      })()}
                      rows={expandedChart.rows.slice((expandedChart.currentPage - 1) * ROWS_PER_PAGE, expandedChart.currentPage * ROWS_PER_PAGE)}
                      maxValue={expandedChart.maxValue}
                      compact={false}
                      colorSet={colorSet}
                      rowColors={(() => {
                        // Generate row colors for the current page using the same logic as compact view
                        const pageRows = expandedChart.rows.slice((expandedChart.currentPage - 1) * ROWS_PER_PAGE, expandedChart.currentPage * ROWS_PER_PAGE);
                        const rowColorMap: Record<string, string> = {};
                        
                        // Create color array from color set properties
                        const colorArray = [
                          colorSet.HIGHEST,
                          colorSet.HIGH,
                          colorSet.MEDIUM,
                        ...
                        pageRows.forEach((row, index) => {
                          // Use the same color mapping logic as in compact view
                          const colorIndex = index % colorArray.length;
                          rowColorMap[row] = colorArray[colorIndex];
                        });
                        
                        return rowColorMap;
                      })()}
                      dataType={expandedChart.dataType}
                    />
                  </div>
                </div>
                <ChartExportWidget 
                  chartId={`${expandedChart.type}_expanded_chart`}
                  chartTitle={`${expandedChart.title} - Individual Patient Data`}
                  data={(() => {
                    const exportData: any[] = [];
                    if (expandedChart.scatterData && expandedChart.scatterData.length > 0) {
                      expandedChart.scatterData.forEach((series: any) => {
                        series.data.forEach((point: any) => {
                          exportData.push({
                            Date: point.x,
                            Item: point.y,
                            Occurrences: point.size,
                            Frequency: point.frequency,
                            DataType: expandedChart.type
                          });
                        });
                      });
                    }
