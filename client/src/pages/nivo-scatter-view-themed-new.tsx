...
import { 
  ...
  File, ChevronDown, BarChart3, ZoomIn
} from 'lucide-react';
...
export default function FixedExpandedView() {
  // State for data
              ...
              <div className="flex-grow overflow-hidden">
                <div className="w-full h-full flex flex-col overflow-hidden">
                  ...
                  <div 
                    ...
                    style={{ 
                      height: 'calc(100% - 40px)' // Leave space for the date footer
                    }}
                  >
                    <div
                      style={{ 
                        height: Math.max(500, 22 * (data?.rows.length || 0)) + 'px',
                        width: '100%',
                        position: 'relative'
                      }}
                      ref={chartRef}
                  ...
                  {/* Fixed date labels footer - always visible at bottom */}
                  {data && data.columns && data.columns.length > 0 && (
                    ...
                    >
                      ...
                      >
                        ...
                        <div style={{
                          width: '250px', // Fixed width spacer for Y-axis labels
                          flexShrink: 0
                        }}></div>
                        
                        {/* Container for the actual date labels with proper positioning */}
                        <div 
                          style={{
                            display: 'flex',
                            flex: 1,
                        ...
                        >
                          {/* Date labels with precise positioning */}
                          {data.columns.map((date, i) => {
                            // Calculate position based on number of columns
                            let position;
                            
                            if (data.columns.length === 1) {
                              position = 50; // Center single label
                            } else {
                              // Simple evenly distributed positioning
                              const usableWidth = 92; // Use 92% of width
                              const offset = (100 - usableWidth) / 2;
                              
                              if (data.columns.length === 2) {
                                // Special case for exactly 2 columns
                                position = i === 0 ? 12 : 88;
                              } else if (i === 0) {
                                // First date
                                position = offset;
                    ...
                    >
                      ...
                      {data.columns.map((date, i) => {
                        ...
                        if (data.columns.length === 1) {
                          ...
                          // First label is aligned left, last label is aligned right
                          // Others are distributed evenly between
                          const usableWidth = 96; // Use 96% of width to avoid edge crowding
                          const offset = (100 - usableWidth) / 2; // Margin on each side
                          if (data.columns.length > 1) {
                            if (i === 0) {
                              position = offset + 1; // First label gets slight offset from left
                            } else if (i === data.columns.length - 1) {
                              position = 100 - offset - 1; // Last label gets slight offset from right
                            } else {
                              // Calculate the position for points between first and last
                              const innerWidth = usableWidth - 2; // Account for first/last offsets
                              const step = innerWidth / (data.columns.length - 2);
                              position = offset + 1 + (i - 0) * step;
                            }
                          }
                        }
