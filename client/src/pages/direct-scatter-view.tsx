...
// Define exact colors for the visualization (used before theme implementation)
const EXACT_COLORS = {
  HIGHEST: '#994C99',  // Deep purple 
  HIGH: '#8856A7',     // Dark purple
  MEDIUM: '#8C96C6',   // Purple
...
// Function to determine color for a specific value (original implementation)
...
// Function to determine color based on normalized value using current theme
const getThemeColor = (normalizedValue: number, colors: typeof COLOR_THEMES.iridis): string => {
  ...
  const scatterData = React.useMemo(() => {
    ...
    data.rows.forEach((row) => {
      ...
      data.columns.forEach((column) => {
        ...
        if (value > 0) {
          // Create a data point with the exact color value
          points.push({
            x: column,
            y: row,
            value,
            color: getColor(value, data.maxValue)
          });
        }
      });
    });
...
const DirectScatterChart = ({ 
  ...
  useEffect(() => {
    if (!svgRef.current || !data.length || !rows.length || !columns.length) return;
...
export default function DirectScatterView() {
  ...
  React.useEffect(() => {
    if (!patientId) {
      setLocation(`/direct-scatter-view/${patientToDisplay}`);
    }
