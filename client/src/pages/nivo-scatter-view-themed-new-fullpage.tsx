...
import { Loader2, FileSpreadsheet, FileImage, File, Maximize2, Minimize2 } from 'lucide-react';
...
const ScatterSection = ({ 
  ...
  // Create patient info header for export
  const createPatientInfoHeader = () => {
    ...
    headerDiv.style.border = '1px solid #e5e7eb';
  ...
  const downloadAsPNG = useCallback(async () => {
    ...
    try {
      ...
      tempWrapper.style.left = '-9999px'; // Position offscreen

      // Add patient info header
      const patientInfoHeader = createPatientInfoHeader();
      tempWrapper.appendChild(patientInfoHeader);

      // Clone the chart element
      const chartElement = chartRef.current.cloneNode(true) as HTMLElement;
      ...
      expandElements(chartElement);
      tempWrapper.appendChild(chartElement);

      // Append to document to render
  ...
  }, [chartRef, dataType, patientId, toast]);
  ...
  const scatterData = React.useMemo(() => {
    ...
    try {
        ...
        data.columns.forEach((column) => {
          ...
          if (value > 0) {
            ...
            allPoints.push({
              ...
              color: rowColors[row] || '#6A0DAD' // Row color
            });
          }
        });
      });
      ...
      console.error(`Error processing scatter data: ${err}`);
      return [];
    }
  }, [data, dataType, maxValue, rowColors, rowTotals]);

  // Color legend to show intensity levels
  const renderColorLegend = () => {
    // Fixed threshold values to match the getExactIridisColor thresholds
    const thresholds = {
      "11+": colorSet.HIGHEST,
      "8-10": colorSet.HIGH,
      "5-7": colorSet.MEDIUM,
    ...
    return (
      <div className="flex flex-wrap gap-2 justify-end mb-1">
        <div className="text-xs text-gray-600 mr-1">Bubble size = occurrences:</div>
        {Object.entries(thresholds).map(([label, color]) => (
  ...
  return (
    <div className="relative">
      ...
      >
        <DialogContent 
          className={isFullPageView 
            ? "w-[95vw] h-[90vh] max-w-[95vw] max-h-[90vh] p-4 overflow-hidden flex flex-col" 
        ...
        >
          ...
          {data && (
            <div className={`flex flex-col ${isFullPageView ? 'h-full' : ''} overflow-hidden`}>
              {!isFullPageView && (
              ...
              <div className="w-full flex flex-col flex-grow">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {isFullPageView ? (
                        <><Minimize2 className="h-4 w-4 mr-1" /> Standard View</>
                      ) : (
                        <><Maximize2 className="h-4 w-4 mr-1" /> Full Page View</>
                      )}
                    </Button>
                  </div>
                  <div className="flex items-center gap-2">
                ...
                <div 
                  className={`${isFullPageView ? "flex-grow" : "overflow-auto"}`}
                  style={{ 
                    height: isFullPageView ? 'calc(80vh - 120px)' : '75vh',
      ...
      <div className="p-2">
        {isLoading ? (
          ...
          <div 
            className="mt-1"
            style={{ 
              height: '300px', // Use same fixed height for both HRSN and Diagnosis
  ...
  const processedData = data.map(series => {
    return {
      ...
      data: series.data.map(point => ({
        ...
        color: point.color // Keep the original color
      }))
    };
  });
    ...
    <div style={{ height: '100%', width: '100%', position: 'relative' }}>
      ...
      <ResponsiveScatterPlot
        ...
        nodeSize={(d: any) => {
          ...
          const scaledSize = Math.max(5, Math.min(23, 5 + Math.floor((occurrences - 1) * 2)));
        ...
        colors={(d: any) => {
          ...
          // When coloring at the node level, we have direct access to the data point
          if (d.data && typeof d.data === 'object') {
            // First try to use the color from the data point itself
            if (d.data.color && typeof d.data.color === 'string') {
              return d.data.color;
            }

            // Extract the row from the data point's y value
            const row = d.data.y;
            // Use the pre-calculated rowColors map
            if (row && rowColors[row]) {
        ...
        onMouseEnter={(node, event) => {
          ...
          if (node && node.data && event) {
            // Keep the row color for highlighting the tooltip
            const rowName = String(node.data.y || '').replace(/ \(\d+\)$/, ''); // Remove count in parentheses
            const rowColor = rowColors[node.data.y] || '#3B82F6';
            ...
            // Set a small delay to avoid tooltip flickering on rapid mouse movement
            tooltipTimerRef.current = window.setTimeout(() => {
              setTooltipData({
                x: String(node.data.x || ''),
                y: String(node.data.y || '').replace(/ \(\d+\)$/, ''), // Remove count from display
                size: Number(node.data.size || 0),
                frequency: Number(node.data.frequency || 0),
                visible: true,
                color: rowColor
              });
...
// Main component that displays the full patient visualization with all data types
export default function NivoScatterViewThemed() {
  ...
  const [colorTheme, setColorTheme] = React.useState<string>('iridis');
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Patient ID: P{patientId.padStart(4, '0')} | Comprehensive Data Visualization
                </CardDescription>
              </div>
                  ...
                  <SelectContent>
                    {Object.keys(COLOR_THEMES).map(key => (
                      <SelectItem key={key} value={key}>
                        {COLOR_THEMES[key as keyof typeof COLOR_THEMES].name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {DATA_TYPES.map(type => (
                <Card key={type.id} className="overflow-hidden p-2">
                  <ScatterSection 
                    dataType={type.id} 
                    patientId={patientId}
