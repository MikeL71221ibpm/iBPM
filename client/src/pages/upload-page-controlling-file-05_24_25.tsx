...
export default function UploadPage() {
  ...
  const [showExtractAlert, setShowExtractAlert] = useState(false);
  const [lastStatsCheck, setLastStatsCheck] = useState(Date.now());
  
  // Real-time progress tracking
  const [realTimeProgress, setRealTimeProgress] = useState(0);
  const [isExtracting, setIsExtracting] = useState(false);
  const [isProcessingInBackground, setIsProcessingInBackground] = useState(false); // Never start with background processing
  const [extractionMessage, setExtractionMessage] = useState("");
  const [justUploaded, setJustUploaded] = useState(false); // Track recent upload state
  const [progressBarStartTime, setProgressBarStartTime] = useState<number | null>(null); // Track when progress bar started
  const [expectedSymptomCount, setExpectedSymptomCount] = useState<number | null>(null); // Track expected symptoms from extraction message
  const [totalExpectedSymptoms, setTotalExpectedSymptoms] = useState<number | null>(null); // Track total expected from completion message
      ...
      try {
        ...
        if (data.type === 'extraction_progress') {
          ...
          if (data.message) {
            ...
            // Extract expected total symptom count from completion message
            if (data.message.includes('Successfully extracted and saved') && data.message.includes('symptoms')) {
              const match = data.message.match(/(\d+,?\d*)\s+symptoms/);
              if (match) {
                const count = parseInt(match[1].replace(/,/g, ''));
                setTotalExpectedSymptoms(count);
                console.log(`📊 Captured expected symptom count: ${count}`);
              }
            }
          }
          ...
          if (data.status === 'in_progress' || (data.progress > 0 && data.progress < 100)) {
            ...
            // Always show progress during active extraction
            setShowPostUploadLoading(true);
            // Set start time if not already set
            setProgressBarStartTime(prev => prev || Date.now());
            ...
            // IMMEDIATELY fetch database stats to trigger fast spinner dismissal
            console.log('✅ Extraction complete - fetching database stats for immediate dismissal');
            setTimeout(() => {
              fetchDatabaseStats();
            }, 500); // Small delay to allow database to finish writing
            
            console.log(`✅ Progress reached 100% at ${new Date().toLocaleTimeString()} - triggering immediate stats refresh`);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // Clear immediately at 100% progress if database is populated
    if (realTimeProgress === 100 && databaseStats.symptomCount > 0) {
      console.log('✅ Progress at 100% with database populated - immediate clear');
      setShowPostUploadLoading(false);
      setIsExtracting(false);
      setTimeout(() => {
        setRealTimeProgress(0);
  ...
  // Clear justUploaded flag after 5 minutes to prevent indefinite progress bar
  useEffect(() => {
    if (justUploaded) {
      const timer = setTimeout(() => {
        console.log('⏱️ Clearing justUploaded flag after 5 minutes');
        setJustUploaded(false);
      }, 300000); // 5 minutes
  ...
  // Function to evaluate smart alert conditions
  const evaluateSmartAlerts = () => {
    const now = Date.now();
    const timeSinceLastCheck = now - lastStatsCheck;
    
    // Calculate indicator extraction percentage
    const indicatorPercentage = databaseStats.noteCount > 0 
      ? (databaseStats.symptomCount / databaseStats.noteCount) * 100 
      : 0;
    
    // Show Refresh Alert if:
    // - System hasn't been refreshed in 5+ minutes AND has data
    // - Processing status shows completed but stats seem stale
    const shouldShowRefresh = (
      (timeSinceLastCheck > 300000 && databaseStats.patientCount > 0) || // 5 minutes
      (databaseStats.processingStatus?.status === "completed" && 
       databaseStats.processingStatus?.progress === 100 && 
       timeSinceLastCheck > 120000) // 2 minutes after completion
    );
    
    // Show Extract Alert if:
    // - Have notes but very low indicator extraction rate (< 5%)
    // - Have patients but zero indicators
    // - Large dataset with minimal extraction (< 2% for enterprise datasets)
    const shouldShowExtract = (
      (databaseStats.noteCount > 0 && indicatorPercentage < 5) ||
      (databaseStats.patientCount > 0 && databaseStats.symptomCount === 0) ||
      (databaseStats.noteCount > 10000 && indicatorPercentage < 2)
    );
    
    setShowRefreshAlert(shouldShowRefresh);
    setShowExtractAlert(shouldShowExtract);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-3 py-2">
        ...
        <div className="mb-3 bg-white shadow rounded-lg p-3">
          <div className="flex justify-between items-center mb-2">
            ...
            {/* Extraction status removed per user request */}
          </div>
      ...
      {/* PostUploadLoading - Shows during upload and extraction, hides immediately at completion */}
      <PostUploadLoading 
        isVisible={
          // Show during upload and extraction (1-99%), hide immediately at 100%
          (realTimeProgress > 0 && realTimeProgress < 100) || isExtracting
        }
